...
Deno.serve(async (req) => {
  try {
    ...
    let invoiceData = (await base44.asServiceRole.entities.Invoice.filter({ id: invoiceId }))[0];
    if (!invoiceData) return Response.json({ error: 'Invoice not found' }, { status: 404 });
    if (invoiceData.seller_email !== user.email && user.role !== 'admin') {
      return Response.json({ error: 'Forbidden' }, { status: 403 });
    }

    const [sellerProfile] = await base44.asServiceRole.entities.SellerProfile.filter({ user_email: invoiceData.seller_email });

    // Always recompute fee_credit and total_amount from item_price to ensure correctness
    const itemPrice = Number(invoiceData.item_price) || 0;
    const serviceFee = Math.round((itemPrice * 0.10 + 30) * 100) / 100;
    const feeCredit  = Math.round(serviceFee * 0.50 * 100) / 100;
    const extraTotal = (invoiceData.additional_line_items || []).reduce((s, li) => {
    ...
    const invoice = { ...invoiceData, fee_credit: feeCredit, total_amount: totalAmount };
